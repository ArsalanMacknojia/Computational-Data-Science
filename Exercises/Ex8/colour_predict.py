...
def convert_rgb_2_hsv(X):
    X = X.reshape(1, -1, 3)
    X = rgb2hsv(X)
    return X.reshape(-1, 3)


def convert_rgb_2_lab(X):
    X = X.reshape(1, -1, 3)
    X = rgb2lab(X)
    return X.reshape(-1, 3)
...
def main():
    ...
    # GaussianNB-based classifier
    X_train, X_valid, y_train, y_valid = train_test_split(X, y)
    bayes_rgb_model = GaussianNB()
    bayes_convert_model = make_pipeline(FunctionTransformer(convert_rgb_2_lab, validate=True), GaussianNB())

    # K-nearest neighbours classifier
    knn_rgb_model = KNeighborsClassifier(n_neighbors=15)
    knn_convert_model = make_pipeline(FunctionTransformer(convert_rgb_2_lab, validate=True),
                                      KNeighborsClassifier(n_neighbors=15))

    # Random forest classifier
    rf_rgb_model = RandomForestClassifier(n_estimators=400, max_depth=7)
    rf_convert_model = make_pipeline(FunctionTransformer(convert_rgb_2_lab, validate=True),
